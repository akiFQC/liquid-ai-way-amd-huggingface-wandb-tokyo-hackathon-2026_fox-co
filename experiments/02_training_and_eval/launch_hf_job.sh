#!/usr/bin/env bash
# Submit the fox-co PII 抽出 fine-tune (experiments/02_training_and_eval/train.py) to
# HuggingFace Jobs. Reads credentials from .env, runs the fail-closed
# gates, then submits with `hf jobs uv run --detach`.
#
# Usage:
#   ./experiments/02_training_and_eval/launch_hf_job.sh                     # defaults (a100-large, 2h timeout)
#   HF_FLAVOR=l40sx1 ./experiments/02_training_and_eval/launch_hf_job.sh
#   MAX_STEPS=500 DATASET=your-org/japanese-pii-sft ./experiments/02_training_and_eval/launch_hf_job.sh
#   DRY_RUN=1 ./experiments/02_training_and_eval/launch_hf_job.sh            # run the gates, skip the submit
#
# Required env (loaded from .env or your shell):
#   HF_TOKEN, WANDB_API_KEY, WANDB_ENTITY, WANDB_PROJECT
#
# Required training env:
#   DATASET          HF dataset id (必須)
#
# Optional env (forwarded to the training script; see train.py docstring):
#   MODEL_ID, DATASET_SLICE, DATASET_MAPPER, MAX_STEPS, BATCH_SIZE,
...
: "${DATASET:?DATASET not set; set to your HF dataset id (e.g. your-org/japanese-pii-sft)}"

HF_FLAVOR="${HF_FLAVOR:-a100-large}"
HF_TIMEOUT="${HF_TIMEOUT:-2h}"
SCRIPT="$REPO_ROOT/experiments/02_training_and_eval/train.py"
...
for var in MODEL_ID DATASET_SLICE DATASET_MAPPER MAX_STEPS MAX_EPOCHS BATCH_SIZE \
  LR OUTPUT_DIR PUSH_TO_HUB WANDB_RUN_NAME EVAL_SPLIT_RATIO EVAL_SAMPLES EVAL_BATCH_SIZE SKIP_TRAINING; do
  if [[ -n "${!var:-}" ]]; then
...
echo "Submitting fox-co PII 抽出 fine-tune:"
